...
#   demo1：遷移學習，以VGG為例    #
###################################
#https://www.youtube.com/watch?v=HWHBCsUR-58&list=PLXO45tsB95cKI5AIlf5TxxFPzb-0zeVZ8&index=44
...
def download():     # download tiger and kittycat image
    ...
    for category in categories:
        root = './Transfer_Learning/'
        path = root + category
        os.makedirs(path, exist_ok=True)
        with open(root + 'imagenet_%s.txt' % category, 'r') as file:
            urls = file.readlines()
            n_urls = len(urls)
            for i, url in enumerate(urls):
                try:
                    urlretrieve(url.strip(), root + '%s/%s' % (category, url.strip().split('/')[-1]))
...
def load_img(path):
    ...
    return resized_img
#%%
test_path = "./Transfer_Learning/image/tiger/_39721973_tiger_ap_203.jpg"
img = skimage.io.imread(test_path)
print(img.shape)
#%%
def load_data():
    imgs = {'tiger': [], 'kittycat': []}
    for k in imgs.keys():
        _dir = './Transfer_Learning/image/' + k
        for file in os.listdir(_dir):
            ...
            try:
                ...
                print("IndexError: ", os.path.join(_dir, file))
...
def train():
    ...
    vgg.save('./SaveModel/transfer_learning')      # save learned fc layers
#%%
train()
#%%
def eval():
    vgg = Vgg16(vgg16_npy_path='./Transfer_Learning/vgg16.npy',
                restore_from='./SaveModel/transfer_learning')
    vgg.predict(
        ['./Transfer_Learning/image/kittycat/000129037.jpg', './Transfer_Learning/image/tiger/391412.jpg'])
#%%
eval()
